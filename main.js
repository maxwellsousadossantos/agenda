...
const listaContatos =
document.getElementById("lista-contatos");
...
async function carregarContatos() {
 ...
 <td class="actions">
 <button
onclick="atualizarContato('${contato.id}')">✏
Atualizar</button>
 <button
onclick="excluirContato('${contato.id}')">🗑 Excluir</button>
 </td>
 `;
...
async function atualizarContato(id) {
 ...
 const { error } = await supabaseClient
 .from("contatos")
 .update({ nome: novoNome, email: novoEmail, telefone:
novoTelefone })
 .eq("id", id);
 if (error) {
